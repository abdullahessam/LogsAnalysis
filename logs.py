...
def createviews():
    ...
    c = db.cursor()
    postsview = '''create or replace view posts as  select log.path ,
    count(log.path) as num
from log  inner join
(select slug from articles) as slug
on path like '%' || slug.slug
group by log.path order by num desc;'''
    allrequestview = '''
      create  or replace view allreq as
      select date(time),count(status) as num
       from log   group by date(time)   ;'''
    errrequestview = '''
      create  or replace view fail as select date(time),
      count(status)  as num from log   where status!='200 OK'
      group by date(time) , status  ;'''
    db = psycopg2.connect("dbname=news")
    c = db.cursor()
    c.execute(postsview)
    c.execute(allrequestview)
    c.execute(errrequestview)
    db.close()
...
def getPopularAuther():
    db = psycopg2.connect("dbname=news")
    c = db.cursor()
    q = """select name , count(slug),sum(num) from authors, articles, posts
...
def getdaterrors():
    ...
    c = db.cursor()
    q = """
    select f.date as day , f.num as fail ,a.num  as all,
    (cast(f.num as float)/
    cast(a.num as float)) *100  as percent
 from allreq as a inner join fail as f
 on a.date=f.date
  where
  (cast(f.num as float)/cast(a.num as float)) *
  100 > 1 order by percent desc
  limit 1 ;
